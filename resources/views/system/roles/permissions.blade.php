                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Adicionar Permissão à {{$item->name}}</h4>
                        ...
                        <div class="card-content collapse show">
                            <div class="card-body card-dashboard">
                                ...
                                <form id="" method="POST" action="{{ route($ctrl['route'].'.sync', $item->id) }}">
                                    {{ csrf_field() }}
                                    <div class="row justify-content-center">
                                        <div class="col-lg-10 col-md-12">
                                            <div class="form-group">
                                                ...
                                                <div class="controls">
                                                    <select class="select2 form-control" name="permission_id[]" multiple="multiple">
                                                        <option value="">Permissões</option>
                                                        @foreach ($permissions as $permission)
                                                            @if(isset($item->permission_id))
                                                                <option value="{{$permission->id}}" {{ $item->permission_id == $permission->id ? 'selected': null }}>{{$permission->name}}</option>
                                                            @else
        ...
        <section id="configuration">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Listagem de Permissões de {{$item->name}}</h4>
                            <a class="heading-elements-toggle"><i class="la la-ellipsis-v font-medium-3"></i></a>
                            <div class="heading-elements">
                                <ul class="list-inline mb-0">
                                    <li><a data-action="collapse"><i class="ft-minus"></i></a></li>
...
@push('scripts')
    ...
    <script>
        ...
        var columnsCustom = [
            { data: 'name', name: 'name' },
            { data: 'description', name: 'descripion' },
            { data: 'area', name: 'area' },
